...
functions {
  vector npmodel (real pmax, real alpha, real beta, real rd, vector x) {
    return pmax * (1-exp(-alpha/pmax * x)) .* exp(-beta/pmax * x) - rd;
  }
  real npmodel_real (real pmax, real alpha, real beta,  real rd, real x) {
    return pmax * (1-exp(-alpha/pmax * x)) * exp(-beta/pmax * x) - rd;
  }
}
...
generated quantities {
  ...
  for(k in 1:K){
    yhat[k] = npmodel(PMAX[k], ALPHA[k], BETA[k], RESP[k], light);
  }
  for(m in 1:M)  {
    for(k in 1:K) {
